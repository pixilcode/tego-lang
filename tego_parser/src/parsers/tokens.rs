use crate::error::parse_handlers::{
    err_retain_all,
...
use crate::{Input, InternalParseResult};
use nom::{
    branch::alt,
...
pub fn comment0(input: Input<'_>) -> InternalParseResult<'_, Vec<Input<'_>>> {
    terminated(
        many0(
...
pub fn multicomment0(input: Input<'_>) -> InternalParseResult<'_, Vec<Input<'_>>> {
    terminated(
...
fn single_comment(input: Input<'_>) -> InternalParseResult<'_, Input<'_>> {
    preceded(
        tag("--"),
...
fn inline_comment(input: Input<'_>) -> InternalParseResult<'_, Input<'_>> {
    verify(
...
fn multi_comment(input: Input<'_>) -> InternalParseResult<'_, Input<'_>> {
    terminated(preceded(tag("{-"), take_until("-}")), tag("-}"))(input)
        .map_err(multi_comment_error(input))
}

pub fn opt_nl<'a, F, O>(parser: F) -> impl Fn(Input<'a>) -> InternalParseResult<'a, O>
where
...
pub fn preceding_opt_nl<'a, F, O>(parser: F) -> impl Fn(Input<'a>) -> InternalParseResult<'a, O>
where
    F: Fn(Input<'a>) -> InternalParseResult<'a, O>,
{
...
pub fn req_nl<'a, F, O>(parser: F) -> impl Fn(Input<'a>) -> InternalParseResult<'a, O>
where
    F: Fn(Input<'a>) -> InternalParseResult<'a, O>,
{
    terminated(
...
macro_rules! reserved {
    (keyword $lexeme:ident, $lexeme_str:literal) => {
        pub fn $lexeme(input: Input<'_>) -> InternalParseResult<'_, Input<'_>> {
            token(terminated(
                tag($lexeme_str),
                alt((
...
pub fn char(input: Input<'_>) -> InternalParseResult<'_, char> {
    token(preceded(
        tag("'"),
        terminated(
...
pub fn string(input: Input<'_>) -> InternalParseResult<'_, String> {
    token(preceded(
        tag("\""),
        terminated(
            inner_string,
...
pub fn identifier(input: Input<'_>) -> InternalParseResult<'_, Input<'_>> {
...
pub fn minus(input: Input<'_>) -> InternalParseResult<'_, Input<'_>> {
    map(
...
pub fn slash(input: Input<'_>) -> InternalParseResult<'_, Input<'_>> {
    map(
        pair(token(tag("/")), peek(nom_not(tag("=")))),
        |(slash_op, _)| slash_op
    )(input)
